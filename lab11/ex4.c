...
int findLeader(int rank, int numProc)
{
    	...
    	for(i = 0; i != numProc; ++i) {
        	...
        	for(j = 0; j != numNeighbours; ++j) {
            		MPI_Send(&leader, 1, MPI_INT, neighbours[j], MPI_TAG_UB, MPI_COMM_WORLD);
            		MPI_Recv(&newLeader, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
